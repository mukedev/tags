...
trait MultiSourceModel {
  ...
  def startFlow(): Unit = {
    ...
    // 3. 读取对应的数据库, 拿到源表数据
    val sourceArr: Array[(DataFrame, CommonMeta)] = createSource(metaData)
    ...
    // 4. 计算标签
    val commonMeta = sourceArr.map(s => s._2).head
    val result = process(sourceArr.map(s => s._1), fiveTag, commonMeta.outFields)
  ...
  def process(sources: Array[DataFrame], fiveTags: Array[Tag], outFields: Array[String]): DataFrame
   ...
   */
  def createSource(metaDataArr: Array[MetaData]): Array[(DataFrame, CommonMeta)] = {

    def createSourceSingle(metaData: MetaData): (DataFrame, CommonMeta) = {
    ...
    metaDataArr.map(createSourceSingle)
  }
