...
trait MultiSourceModel {
  ...
  def startFlow(): Unit = {
    ...
    // 5.匹配计算标签数据
    val commonMeta = sourceArr.map(i => i._2).head
    val result = process(sourceArr.map(i => i._1), fiveTags, commonMeta.outFields)
   ...
   * @return
   */
  def process(source: Array[DataFrame], fiveTags: Array[Tag], outField: Array[String]): DataFrame
   ...
   * 读取并处理元数据
   */
  def readMetaData(tagId: String): Array[MetaData] = {
    // 1.获取数据配置
